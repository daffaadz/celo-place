...
import { ChatDots, PaperPlaneTilt, Coins, SpinnerGap, X } from "@phosphor-icons/react";
    ...
    <>
      ...
      {!isOpen && (
        <button
          ...
          className="absolute bottom-6 right-6 z-[1000] bg-[var(--accent-primary)] text-white w-14 h-14 rounded-full flex items-center justify-center shadow-[0_0_20px_rgba(228,76,255,0.4)] hover:scale-105 transition-transform pointer-events-auto"
        >
          <ChatDots weight="fill" className="w-6 h-6" />
        </button>
      )}
      ...
      <div className={cn(
        ...
        <div className={cn(
          ...
          <div className="flex items-center gap-2">
            <ChatDots weight="fill" className="w-5 h-5 text-[var(--accent-primary)]" />
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {isLoading ? (
            <div className="flex justify-center items-center h-full">
              <SpinnerGap className="w-6 h-6 animate-spin text-[var(--accent-primary)]" />
            </div>
          ) : messages.length === 0 ? (
        ...
        <form onSubmit={handleSend} className={cn(
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="bg-[var(--accent-primary)] text-white px-4 py-2 rounded-lg font-bold hover:brightness-110 disabled:opacity-50 transition-colors flex items-center justify-center min-w-[48px]"
            >
              {isWriting ? <SpinnerGap size={16} className="animate-spin" /> : <PaperPlaneTilt size={16} weight="fill" />}
            </button>
          </div>
        </form>
      </div>
